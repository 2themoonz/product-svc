...
export class ProductService {
  ...
  public async fineOne({ id }: FindOneRequestDto): Promise<FindOneResponse> {
  ...
  public async decreaseStock({
    ...
    const isAlreadyDecreased: number =
      await this.stockDecreaseLogRepository.count({
        where: { orderId },
      });
    ...
    await this.stockDecreaseLogRepository.insert({
      product,
      orderId,
    });
